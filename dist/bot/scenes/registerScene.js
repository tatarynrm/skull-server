...
const registerScene = new telegraf_1.Scenes.WizardScene("register-wizard", async (ctx) => {
    ...
    if (ctx.message && "text" in ctx.message) {
        ...
        if (age < 14) {
            await ctx.reply("Ти занадто малий для реєстрації. Будь ласка, введи коректний вік.");
            return;
        }
        if (age > 80) {
            await ctx.reply("Будь ласка, вкажіть коректний вік, оскільки це занадто великий вік для реєстрації.");
            return;
        }
    ...
    if (ctx.message && "text" in ctx.message) {
        ...
        if (geocodedAddress === "Місто не знайдено." ||
    ...
    if (ctx.message && "photo" in ctx.message) {
        ...
        if (photoCount >= 1) {
            ...
            await ctx.replyWithPhoto({ url: ctx.scene.session.registrationData.photos[0].url }, {
                caption: profileText,
                reply_markup: {
                    keyboard: [
                        [{ text: "⚙ Налаштування" }],
  ...
  `, [
            ...
            await ctx.reply("Ти перейшов до знайомств!", {
                reply_markup: main_keyboard_1.MainKeyboard
            });
            return ctx.scene.leave();
        }
    }
});
exports.default = registerScene;
